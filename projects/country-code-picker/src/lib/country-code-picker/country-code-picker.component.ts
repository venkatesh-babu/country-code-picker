  ...
  providers: [
    {
      ...
      useValue: { overlayPanelClass: 'custom-class' },
    },
  ],
})
export class CountryCodePickerComponent {
  ...
  displayFn(data: data): string {
    return data && data.name ? data.name : '';
  }

  selectionChangeForContactMobileCc(name: string) {
    const defaultValue = this.countryCodeData.find(
      (option: any) => option.name == name
    );
    this.selectedValue = defaultValue[this.displayValue];
    this.selectEvent(defaultValue);
  }
  selectEvent(val: any) {
    this.actionSelectEvent.emit({ value: val });
  }
}
